          ...
          <a-table :columns="columns"
            ...
            <template #action="record">
              <div class="action-buttons-wrapper">
                <!-- 复制路径 -->
                <a class="action-icon" @click="$copy(record.path, '已复制文件路径')" title="复制路径">
                  <a-icon type="copy"/>
                </a>
                <!-- 下载 -->
                <a-popconfirm v-if="record.isDir"
                  ...
                  <a class="action-icon" title="下载" @click="clickDownloadTrigger">
                    <a-icon type="cloud-download"/>
                  </a>
                </a-popconfirm>
                <a v-else class="action-icon" @click="download(record)" title="下载">
                  <a-icon type="cloud-download"/>
                </a>
                <!-- 删除 -->
                <a class="action-icon" :class="{'disabled': !record.isSafe}" :title="record.isSafe ? '删除' : '禁止删除'" @click="record.isSafe ? remove(record) : null">
                  <a-icon type="delete"/>
                </a>
                <!-- 移动 -->
                <a class="action-icon" :class="{'disabled': !record.isSafe}" :title="record.isSafe ? '移动' : '无法移动'" @click="record.isSafe ? openMove(record) : null">
                  <a-icon type="block"/>
                </a>
                <!-- 提权 -->
                <a class="action-icon" @click="openChmod(record)" title="提权">
                  <a-icon type="team"/>
                </a>
              </div>
            </template>
...
export default {
  ...
  watch: {
    ...
    // 添加 machineId 监听，当机器切换时重新加载
    machineId: {
      handler(newId, oldId) {
        if (newId && newId !== oldId) {
          // 机器ID变化，重新打开SFTP
          this.openSftpByMachineId(newId)
        }
      },
      immediate: false
    }
  ...
  methods: {
    // 根据机器ID打开SFTP（用于切换机器时）
    openSftpByMachineId(machineId) {
      this.cleanData()
      this.loading = true
      ...
      this.$api.sftpOpen({
        ...
        this.loading = false
        this.$message.error(`机器 SFTP 加载失败`)
      })
    },
