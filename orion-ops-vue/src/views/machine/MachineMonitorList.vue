    ...
    <div class="table-wrapper">
      ...
      <div class="table-tools-bar">
        ...
        <div class="tools-fixed-right">
          <a href="https://om.orionsec.cn/quickstart/download.html" target="_blank" class="link-btn">安装文档</a>
          <a-popconfirm title="确定执行批量安装本页监控插件?" ok-text="确定" cancel-text="取消" @confirm="batchInstallMonitor">
            <a-button type="primary" icon="cloud-server" class="action-btn">安装</a-button>
          </a-popconfirm>
          <a-popconfirm title="确定执行批量升级本页监控插件?" ok-text="确定" cancel-text="取消" @confirm="batchUpgradeMonitor">
            <a-button type="primary" icon="cloud-upload" class="action-btn">升级</a-button>
          </a-popconfirm>
          <a-popconfirm title="确定执行批量检测本页监控插件状态?" ok-text="确定" cancel-text="取消" @confirm="batchCheckMonitor">
            <a-button type="primary" icon="cloud-sync" class="action-btn">检测</a-button>
          </a-popconfirm>
      ...
      <div class="table-main-container table-scroll-x-auto">
        <a-table :columns="columns"
          ...
          <template #version="record">
            <span v-if="record.currentVersion" class="span-blue">V{{ record.currentVersion }}</span>
            <span v-else>-</span>
            ...
            record.currentVersion && record.latestVersion && record.currentVersion !== record.latestVersion">
              <template #title>最新版本: V{{ record.latestVersion }} 点击进行升级</template>
              <a-tag class="ml4 pointer usn" @click="upgradeMonitor(record)">升级</a-tag>
            </a-tooltip>
          </template>
          <!-- 操作 -->
          <template #action="record">
            <a-button class="p0" v-if="MONITOR_STATUS.NOT_START.value === record.status" type="link" style="height: 22px" @click="installMonitor(record)">安装</a-button>
            <a-button class="p0" v-else type="link" style="height: 22px" :disabled="MONITOR_STATUS.RUNNING.value !== record.status">
